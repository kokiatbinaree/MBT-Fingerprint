    ...
    <div class="pa-3 content background-main">
      <v-row justify="center" class="px-3 mt-5">
        <v-card tile style="width: 97%; min-height: 82vh">
          <v-card-title class="pt-7 pb-1 pl-10 pr-7">
            <span style="font-size: 1.5rem">นำเข้าไฟล์ข้อมูล</span>
            <v-spacer></v-spacer>
            <v-btn color="#AACF48" dark @click="exportInformation">Export Information</v-btn>
          </v-card-title>
          <v-card-actions>
                      ...
                      >
                        <span>
                          ...
                          <span style="font-weight: 400; color: #707070;">
                            {{ `(${importCompletedList.length} Data)` }}
                          </span>
                        </span>
                            ...
                            <v-list-item>
                              <v-list-item-content>
                                <v-list-item-title class="text-left px-3">
                                  {{ `${item.mb_code} ${item.group} ${item.type} ${item.status}` }}
                                </v-list-item-title>
                              </v-list-item-content>
                            </v-list-item>
                          </v-list>
  ...
  methods: {
    ...
    async importData() {
      if (this.fileUpload !== null) {
        ...
        this.importDataWithApi(fileUrl);
      }
    },
    async uploadFileToStorage(file) {
      return new Promise(function (resolve, reject) {
        ...
        var storagePath = "import-information";
        var fileName = "import-information.xlsx";
    ...
    importDataWithApi(fileUrl) {
      let header = { access_token: this.userProfile.access_token };
      axios
        .post(
          apiUrl + "/informations/import_data",
          { import_url: fileUrl },
          { headers: header }
        )
        .then((response) => {
          ...
          if (data.status == "success") {
            ...
            if (this.importCompletedList.length !== 0 && this.importFailList.length === 0) {
              this.panelCompleted = 0
              this.panelFail = undefined
            } else if (this.importCompletedList.length === 0 && this.importFailList.length !== 0) {
    ...
    exportInformation() {
      ...
      let header = {'access_token': this.userProfile.access_token}
      axios
      .post(apiUrl + "/informations/export_data",'', { headers: header })
      .then((response) => {
