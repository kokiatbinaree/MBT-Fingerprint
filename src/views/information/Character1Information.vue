<template>
  <div class="Character1Information" v-if="userProfile !== null">
    <div class="pa-3 content background-main">
      <v-row justify="space-between" class="px-3">
        <div>
          <p class="font-20 font-bold">ลักษณะนิสัย 1</p>
        </div>
        <div>
          <v-btn color="#A3CB38" dark @click="openCharacter1Dialog('create')">
            Create Information
          </v-btn>
          <v-dialog
            v-if="renderCharacter1Dialog"
            v-model="openingCharacter1Dialog"
            persistent
            max-width="700"
          >
            <form-character
              @CharacterDialog="closingCharacter1Dialog"
              :templateData="templateData"
              :dialogMode="dialogMode"
              :dataInfo="dataInfo"
              @CreateCharacterDialog="createCharacter1Dialog"
              @EditCharacterDialog="editCharacter1Dialog"
            ></form-character>
          </v-dialog>
        </div>
      </v-row>
      <v-row class="px-3 mb-4">
        <div>
          <v-btn color="#2b8152" dark @click="deleteCharacter1()">
              ...
              <td class="text-center pa-3">
                <div class="height-100">
                  <v-btn
                    ...
                    elevation="0"
                    x-small
                    @click="openCharacter1Dialog('edit', item)"
                  >
...
export default {
  ...
  data() {
    return {
      ...
      openingCharacter1Dialog: false,
      renderCharacter1Dialog: true,
      templateData: {
        open: {
          age_range: true,
          header: true,
  ...
  methods: {
    ...
    openCharacter1Dialog(mode, data = null) {
      this.dialogMode = mode;
      this.dataInfo = data || null;
      this.openingCharacter1Dialog = true;
    },
    async createCharacter1Dialog(data) {
      this.setLoading(true);
    ...
    closingCharacter1Dialog(statusDialog) {
      this.openingCharacter1Dialog = statusDialog;
      this.renderCharacter1Dialog = false;
      this.$nextTick(() => {
        this.renderCharacter1Dialog = true;
      });
    },
  },
};
