<template>
  <div class="ActivitiesByPotentialInformation" v-if="userProfile !== null">
    <div class="pa-3 content background-main">
      <v-row justify="space-between" class="px-3">
        <div>
          <p class="font-20 font-bold">กิจกรรมตามค่าศักยภาพ</p>
        </div>
        <div>
          <v-btn
            color="#A3CB38"
            dark
            @click="openActivitiesByPotentialDialog('create')"
          >
            Create Information
          </v-btn>
          <v-dialog
            v-if="renderActivitiesByPotentialDialog"
            v-model="openingActivitiesByPotentialDialog"
            persistent
            max-width="700"
          >
            <form-character
              @CharacterDialog="closingActivitiesByPotentialDialog"
              :templateData="templateData"
              :dialogMode="dialogMode"
              :dataInfo="dataInfo"
              @CreateCharacterDialog="createActivitiesByPotentialDialog"
              @EditCharacterDialog="editActivitiesByPotentialDialog"
            ></form-character>
          </v-dialog>
        </div>
      </v-row>
      <v-row class="px-3 mb-4">
        <div>
          <v-btn color="#2b8152" dark @click="deleteActivitiesByPotential()">
        ...
        >
          <template v-slot:item="{ item, isSelected, select }">
            <tr>
              ...
              <td class="text-center pa-3" style="max-width: 300px">
                <div class="height-100 hard_break" style="max-width: 300px">
                  <span
              ...
              <td class="text-center pa-3">
                <div class="height-100">
                  <v-btn
                    ...
                    elevation="0"
                    x-small
                    @click="openActivitiesByPotentialDialog('edit', item)"
                  >
...
export default {
  name: "ActivitiesByPotentialInformation",
  components: {
    FormCharacter,
  },
  data() {
    return {
      ...
      openingActivitiesByPotentialDialog: false,
      renderActivitiesByPotentialDialog: true,
      templateData: {
        open: {
          age_range: false,
          header: true,
  ...
  methods: {
    ...
    openActivitiesByPotentialDialog(mode, data = null) {
      this.dialogMode = mode;
      this.dataInfo = data || null;
      this.openingActivitiesByPotentialDialog = true;
    },
    async createActivitiesByPotentialDialog(data) {
      this.setLoading(true);
    ...
    editActivitiesByPotentialDialog(data) {
      this.setLoading(true);
      let header = { access_token: this.userProfile.access_token };
      data.age_range = "0";
      axios
    ...
    closingActivitiesByPotentialDialog(statusDialog) {
      this.openingActivitiesByPotentialDialog = statusDialog;
      this.renderActivitiesByPotentialDialog = false;
      this.$nextTick(() => {
        this.renderActivitiesByPotentialDialog = true;
      });
    },
  },
};
