<template>
  <div class="Character2Information" v-if="userProfile !== null">
    <div class="pa-3 content background-main">
      <v-row justify="space-between" class="px-3">
        <div>
          <p class="font-20 font-bold">ลักษณะนิสัย 2</p>
        </div>
        <div>
          <v-btn color="#A3CB38" dark @click="openCharacter2Dialog('create')">
            Create Information
          </v-btn>
          <v-dialog
            v-if="renderCharacter2Dialog"
            v-model="openingCharacter2Dialog"
            persistent
            max-width="700"
          >
            <form-character
              @CharacterDialog="closingCharacter2Dialog"
              :templateData="templateData"
              :dialogMode="dialogMode"
              :dataInfo="dataInfo"
              @CreateCharacterDialog="createCharacter2Dialog"
              @EditCharacterDialog="editCharacter2Dialog"
            ></form-character>
          </v-dialog>
        </div>
      </v-row>
      <v-row class="px-3 mb-4">
        <div>
          <v-btn color="#2b8152" dark @click="deleteCharacter2()">
              ...
              <td class="text-center pa-3">
                <div class="height-100">
                  <v-btn
                    ...
                    elevation="0"
                    x-small
                    @click="openCharacter2Dialog('edit', item)"
                  >
...
export default {
  ...
  data() {
    return {
      ...
      openingCharacter2Dialog: false,
      renderCharacter2Dialog: true,
      templateData: {
        open: {
          age_range: true,
          header: true,
        ...
        age_range: ["000T017", "018T200"],
        group: ["overall_charecteristics", "personality"],
        filterOption: [
          {
            header: "Age range",
            header_value: "age_range",
            option: ["All", "000T017", "018T200"],
          },
          {
            header: "Group",
            header_value: "group",
            option: ["All", "overall_charecteristics", "personality"],
          },
        ],
      },
      dialogMode: "",
  ...
  methods: {
    ...
    async getInformation() {
      ...
      let config = {
        ...
        params: {
          groups: "overall_charecteristics,personality",
          filters: this.filter,
        },
      };
      axios
    ...
    openCharacter2Dialog(mode, data = null) {
      this.dialogMode = mode;
      this.dataInfo = data || null;
      this.openingCharacter2Dialog = true;
    },
    async createCharacter2Dialog(data) {
      this.setLoading(true);
    ...
    closingCharacter2Dialog(statusDialog) {
      this.openingCharacter2Dialog = statusDialog;
      this.renderCharacter2Dialog = false;
      this.$nextTick(() => {
        this.renderCharacter2Dialog = true;
      });
    },
    sendFilter(filter) {
      this.filter =
