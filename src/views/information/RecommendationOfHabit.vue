<template>
  <div class="RecommendationOfHabitInformation" v-if="userProfile !== null">
    <div class="pa-3 content background-main">
      <v-row justify="space-between" class="px-3">
        <div>
          <p class="font-20 font-bold">คำแนะนำตามลักษณะนิสัย</p>
        </div>
        <div>
          <v-btn
            color="#A3CB38"
            dark
            @click="openRecommendationOfHabitDialog('create')"
          >
            Create Information
          </v-btn>
          <v-dialog
            v-if="renderRecommendationOfHabitDialog"
            v-model="openingRecommendationOfHabitDialog"
            persistent
            max-width="700"
          >
            <form-character
              @CharacterDialog="closingRecommendationOfHabitDialog"
              :templateData="templateData"
              :dialogMode="dialogMode"
              :dataInfo="dataInfo"
              @CreateCharacterDialog="createRecommendationOfHabitDialog"
              @EditCharacterDialog="editRecommendationOfHabitDialog"
            ></form-character>
          </v-dialog>
        </div>
      </v-row>
      <v-row class="px-3 mb-4">
        <div>
          <v-btn color="#2b8152" dark @click="deleteRecommendationOfHabit()">
        ...
        <div>
          <FilterField
            :filterOption="templateData.filterOption"
            @FilterReturn="sendFilter"
          ></FilterField>
        </div>
              ...
              <td class="text-center pa-3">
                <div class="height-100">
                  <v-btn
                    ...
                    elevation="0"
                    x-small
                    @click="openRecommendationOfHabitDialog('edit', item)"
                  >
...
import FilterField from "../../components/information/FilterField";

axios.defaults.headers.common["Access-Control-Allow-Origin"] = "*";

export default {
  name: "RecommendationOfHabitInformation",
  components: {
    FormCharacter,
    FilterField,
  },
  data() {
    return {
      ...
      openingRecommendationOfHabitDialog: false,
      renderRecommendationOfHabitDialog: true,
      templateData: {
        open: {
          age_range: true,
          header: true,
        ...
        age_range: ["000T200", "000T017", "018T200"],
        group: ["main_character"],
        filterOption: [
          {
            header: "Age range",
            header_value: "age_range",
            option: ["All", "000T200", "000T017", "018T200"],
          },
        ],
      },
      dialogMode: "",
  ...
  methods: {
    ...
    openRecommendationOfHabitDialog(mode, data = null) {
      this.dialogMode = mode;
      this.dataInfo = data || null;
      this.openingRecommendationOfHabitDialog = true;
    },
    async createRecommendationOfHabitDialog(data) {
      this.setLoading(true);
    ...
    editRecommendationOfHabitDialog(data) {
      this.setLoading(true);
      let header = { access_token: this.userProfile.access_token };
      axios
    ...
    closingRecommendationOfHabitDialog(statusDialog) {
      this.openingRecommendationOfHabitDialog = statusDialog;
      this.renderRecommendationOfHabitDialog = false;
      this.$nextTick(() => {
        this.renderRecommendationOfHabitDialog = true;
      });
    },
    newLineText(text) {
    ...
    sendFilter(filter) {
      this.filter =
        filter.header.length !== 0 && filter.value.length !== 0
