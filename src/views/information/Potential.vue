<template>
  <div class="PotentialInformation" v-if="userProfile !== null">
    <div class="pa-3 content background-main">
      <v-row justify="space-between" class="px-3">
        <div>
          <p class="font-20 font-bold">กราฟค่าศักยภาพ และศักยภาพ 10 ด้าน</p>
        </div>
        <div>
          <v-btn
            color="#A3CB38"
            dark
            @click="openPotentialDialog('create')"
          >
            Create Information
          </v-btn>
          <v-dialog
            v-if="renderPotentialDialog"
            v-model="openingPotentialDialog"
            persistent
            max-width="700"
          >
            <form-character
              @CharacterDialog="closingPotentialDialog"
              :templateData="templateData"
              :dialogMode="dialogMode"
              :dataInfo="dataInfo"
              @CreateCharacterDialog="createPotentialDialog"
              @EditCharacterDialog="editPotentialDialog"
            ></form-character>
          </v-dialog>
        </div>
      </v-row>
      <v-row class="px-3 mb-4">
        <div>
          <v-btn color="#2b8152" dark @click="deletePotential()">
              ...
              <td class="text-center pa-3" style="min-width: 100px">
                <div class="height-100">
                  <v-btn
                    ...
                    elevation="0"
                    x-small
                    @click="openPotentialDialog('edit', item)"
                  >
              ...
              <v-row class="my-3 mx-2 justify-end">
                ...
                <v-col class="col-auto py-1">
                  <v-btn
                    icon
                    :disabled="
                      buttonPageStatus ? true : page === 1 ? true : false
                    "
                    class="mr-1"
                    @click="page--"
                  >
...
export default {
  ...
  data() {
    return {
      ...
      openingPotentialDialog: false,
      renderPotentialDialog: true,
      templateData: {
        open: {
          age_range: false,
          header: true,
  ...
  methods: {
    ...
    openPotentialDialog(mode, data = null) {
      this.dialogMode = mode;
      this.dataInfo = data || null;
      this.openingPotentialDialog = true;
    },
    async createPotentialDialog(data) {
      this.setLoading(true);
    ...
    addClassInHeaderSelect() {
      ...
      let style = "position: sticky !important;position: -webkit-sticky !important;left: 0;z-index: 3;background: #2b8152;";
    ...
    closingPotentialDialog(statusDialog) {
      this.openingPotentialDialog = statusDialog;
      this.renderPotentialDialog = false;
      this.$nextTick(() => {
        this.renderPotentialDialog = true;
      });
    },
  },
};
