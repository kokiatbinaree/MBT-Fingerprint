<template>
  <div class="AwarenessChannelInformation" v-if="userProfile !== null">
    <div class="pa-3 content background-main">
      <v-row justify="space-between" class="px-3">
        <div>
          <p class="font-20 font-bold">ช่องทางการรับข้อมูล</p>
        </div>
        <div>
          <v-btn
            color="#A3CB38"
            dark
            @click="openAwarenessChannelDialog('create')"
          >
            Create Information
          </v-btn>
          <v-dialog
            v-if="renderAwarenessChannelDialog"
            v-model="openingAwarenessChannelDialog"
            persistent
            max-width="700"
          >
            <form-character
              @CharacterDialog="closingAwarenessChannelDialog"
              :templateData="templateData"
              :dialogMode="dialogMode"
              :dataInfo="dataInfo"
              @CreateCharacterDialog="createAwarenessChannelDialog"
              @EditCharacterDialog="editAwarenessChannelDialog"
            ></form-character>
          </v-dialog>
        </div>
      </v-row>
      <v-row class="px-3 mb-4">
        <div>
          <v-btn color="#2b8152" dark @click="deleteAwarenessChannel()">
        ...
        >
          ...
          <template v-slot:item="{ item, isSelected, select }">
            <tr>
              ...
              <td class="text-center pa-3" style="min-width: 150px">
                <div class="height-100">
                  <span v-if="item.image !== '' && item.image !== null">
              ...
              <td class="text-center pa-3" style="min-width: 100px">
                <div class="height-100">
                  <v-btn
                    ...
                    elevation="0"
                    x-small
                    @click="openAwarenessChannelDialog('edit', item)"
                  >
...
export default {
  name: "AwarenessChannelInformation",
  components: {
    FormCharacter,
    FilterField,
  },
  data() {
    return {
      ...
      openingAwarenessChannelDialog: false,
      renderAwarenessChannelDialog: true,
      templateData: {
        open: {
          age_range: false,
          header: true,
  ...
  methods: {
    ...
    async getInformation() {
      ...
      let config = {
        ...
        params: {
          groups:
            "learning_style_kinesthetic,learning_style_auditory,learning_style_visual",
          filters: this.filter,
        },
      };
      axios
    ...
    openAwarenessChannelDialog(mode, data = null) {
      this.dialogMode = mode;
      this.dataInfo = data || null;
      this.openingAwarenessChannelDialog = true;
    },
    async createAwarenessChannelDialog(data) {
      this.setLoading(true);
    ...
    editAwarenessChannelDialog(data) {
      this.setLoading(true);
      let header = { access_token: this.userProfile.access_token };
      data.age_range = "0";
      axios
      ...
      if (this.informationSelected.length !== 0) {
        this.$swal({
          ...
          if (result.isConfirmed) {
            ...
            this.$store.dispatch('deleteImageFromStorage', data)
    ...
    closingAwarenessChannelDialog(statusDialog) {
      this.openingAwarenessChannelDialog = statusDialog;
      this.renderAwarenessChannelDialog = false;
      this.$nextTick(() => {
        this.renderAwarenessChannelDialog = true;
      });
    },
    sendFilter(filter) {
      this.filter =
